...
- **Triggerer** — отдельный процесс для выполнения асинхронных Python-функций в рамках trigger-классов. Нужен для [deferrable-операторов](../04.%20astronomer-advanced/deferrable-operators.md) и [event-driven scheduling](../04.%20astronomer-advanced/event-driven-scheduling.md).
...
3. Воркер, взявший экземпляр задачи, выполняет его; метаданные (статус экземпляра, [XCom](../02.%20astronomer-dags/passing-data-between-tasks.md)) отправляются с воркера через API server в БД метаданных Airflow. Если задаче нужны данные (например, [Airflow connection](../01.%20astronomer-basic/connections.md)), воркер запрашивает их у API server; сервер получает их из БД метаданных и передаёт воркеру. Пока экземпляр задачи выполняется, воркер пишет логи напрямую в настроенное хранилище логов.
...
При настройке продакшен-окружения важно учитывать масштабирование. См. [Scaling out Airflow](scaling-airflow.md).
