...
- Основы Airflow: написание DAG и определение задач. См. [Get started with Apache Airflow](https://www.astronomer.io/docs/learn/get-started-with-airflow).
...
1. Чтобы создать [подключение Airflow](https://www.astronomer.io/docs/learn/connections) к AWS S3, добавьте в `.env` переменную окружения (подставьте свои учётные данные AWS; для другого хранилища измените тип и параметры подключения):
...
2. В UI запустите DAG `object_storage_use_case` кнопкой play. Укажите любую цитату в [параметре Airflow](https://www.astronomer.io/docs/learn/airflow-params) `my_quote`.
