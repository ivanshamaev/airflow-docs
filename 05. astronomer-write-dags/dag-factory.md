...
При сложной условной логике, ветвлении или тонкой обработке ошибок, которую неудобно выражать в YAML, обычно лучше нативный Python. Отлаживать DAG в YAML сложнее: нет пошаговой отладки и богатого логирования. Имеет смысл учитывать опыт команды и существующие практики. [Asset-aware scheduling](../01.%20astronomer-basic/assets.md) в DAG Factory поддерживается, но может быть менее удобен, чем в нативном Python.

## Необходимая база

Полезно понимать:

- [Операторы Airflow](../01.%20astronomer-basic/operators.md).
- [Основы Airflow](../01.%20astronomer-basic/README.md): написание DAG и определение задач.
- [Компоненты Airflow](../03.%20astronomer-infra/airflow-components.md) и их взаимодействие.
...
Примерный DAG будет использовать и [TaskFlow API](../02.%20astronomer-dags/airflow-decorators.md) (декоратор задачи), и [традиционный оператор](../01.%20astronomer-basic/operators.md) (BashOperator). DAG Factory поддерживает оба варианта; туториал ориентирован на Airflow 3.x и по возможности использует TaskFlow API.
...
Рассмотрим [asset-aware scheduling](../01.%20astronomer-basic/assets.md) и его настройку через DAG Factory: один DAG (producer) обновляет asset, второй (consumer) запускается по обновлению этого asset.
...
Упрощённый вариант — [@asset синтаксис](../01.%20astronomer-basic/assets.md): декоратор `@asset(schedule="@daily")` у функции `_update_iss_coordinates` в Python позволяет не описывать DAG producer в YAML. Здесь оба DAG в YAML для демонстрации работы DAG Factory с producer и consumer.
...
DAG Factory поддерживает [динамический маппинг задач](../02.%20astronomer-dags/dynamic-tasks.md): параллельные задачи создаются в рантайме. Пример с TaskFlow API. Пусть в `include/tasks/dtm_tasks.py` определены функции:
...
Выше DAG создавались из статичных YAML. Если нужно много DAG с похожей структурой, конфигурации можно [генерировать динамически](../04.%20astronomer-advanced/dynamic-dags.md) по шаблону YAML.
