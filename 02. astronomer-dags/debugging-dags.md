...
> Рекомендуется наладить систематическое тестирование DAG, чтобы предотвращать типичные ошибки. См. [Test Airflow DAGs](https://www.astronomer.io/docs/learn/testing-airflow).
...
- Основы Airflow. См. [Get started with Airflow tutorial](https://www.astronomer.io/docs/learn/get-started-with-airflow).
...
- Можете ли собрать нужные логи? Где они лежат и как настраиваются: [Airflow logging](https://www.astronomer.io/docs/learn/logging).
...
- В каком состоянии [компоненты Airflow](https://www.astronomer.io/docs/learn/airflow-components)? Проверьте логи каждого компонента и при необходимости перезапустите окружение.
...
Через плагин можно настроить [Airflow listener](https://www.astronomer.io/docs/learn/using-airflow-plugins#airflow-listeners), который выполняет код при появлении новой ошибки импорта (`on_new_dag_import_error`) или при повторной обработке известной ошибки (`on_existing_dag_import_error`).
...
- [ExternalPythonOperator](https://www.astronomer.io/docs/learn/airflow-isolated-environments): задача в заранее созданном виртуальном окружении.
- [KubernetesPodOperator](https://www.astronomer.io/docs/learn/kubepod-operator): задача в отдельном Pod в Kubernetes.
...
- При большом числе экземпляров задач или DAG учитывайте параметры масштабирования и лимиты по умолчанию. Подробнее: [Scaling Airflow](https://www.astronomer.io/docs/learn/airflow-scaling-workers).
...
> После исправления может понадобиться перезапуск DAG или задач. См. [Rerunning DAGs](https://www.astronomer.io/docs/learn/rerunning-dags).

### Проблемы с динамически маппленными задачами

[Динамический маппинг задач](https://www.astronomer.io/docs/learn/dynamic-tasks) позволяет менять число задач в рантайме по входным параметрам. Поддерживается и [динамический маппинг по task groups](https://www.astronomer.io/docs/learn/task-groups#generate-task-groups-dynamically-at-runtime).
...
При сложных сценариях с динамическим маппингом сначала соберите структуру DAG из EmptyOperator или задекорированных Python-операторов, убедитесь, что она верна, затем подставляйте нужные задачи. Примеры: [Create dynamic Airflow tasks](https://www.astronomer.io/docs/learn/dynamic-tasks).

> Часто вывод вышестоящего оператора не совпадает по формату с тем, что нужно для маппинга. Используйте [`.map()`](https://www.astronomer.io/docs/learn/dynamic-tasks#transform-outputs-with-map) для преобразования элементов списка через Python-функцию.
...
- Попробуйте перезапустить задачу через [clearing task instance](https://www.astronomer.io/docs/learn/rerunning-dags#manually-rerun-tasks-or-dags) — иногда логи появляются при повторном запуске.
