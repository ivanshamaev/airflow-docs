...
Экземпляр нотификатора передаётся в любой [параметр callback](#airflow-callbacks); при срабатывании callback’а будет отправлено письмо. Чтобы подставить данные DAG run, используйте [Jinja-шаблоны](https://www.astronomer.io/docs/learn/templating). Все перечисленные параметры, кроме `smtp_conn_id`, поддерживают шаблоны.
