...
| **catchup** | Нужно ли планировщику создавать пропущенные DAG run между текущей датой и start_date при включении DAG. По умолчанию: `False`. Подробнее: [Catchup](https://www.astronomer.io/docs/learn/rerunning-dags#catchup). |

## Параметры отображения в UI

Часть параметров [добавляет описание](https://www.astronomer.io/docs/learn/custom-airflow-ui-docs-tutorial) DAG или меняет его вид в Airflow UI:

| Параметр | Описание |
| --- | --- |
| **description** | Короткая строка, отображаемая в UI рядом с именем DAG. |
| **doc_md** | Строка, которая показывается как [документация DAG](https://www.astronomer.io/docs/learn/custom-airflow-ui-docs-tutorial) в UI. Совет: можно использовать `__doc__`, чтобы подставить docstring файла. Рекомендуется задавать осмысленную документацию для всех DAG. |
| **tags** | Список тегов для фильтрации DAG в UI. |

## Параметры Jinja-шаблонов

Параметры, связанные с [Jinja-шаблонированием](https://www.astronomer.io/docs/learn/templating):
...
| **user_defined_macros** | Словарь макросов, доступных в Jinja-шаблонах DAG. Для фильтров используйте `user_defined_filters`, для дополнительной настройки Jinja — `jinja_environment_kwargs`. См. [Макросы: свои функции и переменные в шаблонах](https://www.astronomer.io/docs/learn/templating#macros-using-custom-functions-and-variables-in-templates). |

## Масштабирование

Следующие параметры влияют на использование ресурсов DAG. Подробнее: [Scaling Airflow](https://www.astronomer.io/docs/learn/airflow-scaling-workers).
...
| **params** | Словарь параметров уровня DAG (Airflow params). Подробнее: [Airflow params](https://www.astronomer.io/docs/learn/airflow-params). |
...
| **fail_fast** | При `True` выполнение DAG останавливается при первой упавшей задаче. Все ещё выполняющиеся задачи помечаются как `failed`, ещё не запущенные — как `skipped`. В DAG с `fail_fast=True` нельзя использовать [trigger rule](https://www.astronomer.io/docs/learn/airflow-trigger-rules), отличные от `all_success`. |
